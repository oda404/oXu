...
public:
	...
	{
		return hitObjectsCurvePointsPositions;
	}

	std::vector<int> getSlides() const
	{
		return numberOfSliderSlides;
	}

	std::vector<float> getSliderLength() const
	{
	...
	void eraseType()
	{
		hitObjectsCurveType.erase(hitObjectsCurveType.begin() + 0);
	}
