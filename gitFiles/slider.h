...
public:
    ...
    void moveOnStraightPath(const float &dt,const float &slideTime,const PlayField &playField, ApproachCircle &approachCircle)
	{
		if (approachCircle.getApproachState())
		{
            sf::Vector2f distance = {sliderPointsCoords[0] - initialPosition};
            
            //questionable solution until i find another one======================================================================================
            if(sliderPointsCoords[0].x > initialPosition.x)
            {
            ...
            else if(sliderPointsCoords[0].x < initialPosition.x)
            {
                if( std::round(((this->hitCircle.getPosition() - playField.getPlayFieldStartPoint() )/playField.getOsuPx()).x) >= sliderPointsCoords[0].x && slides > 0 )
                {
                    sf::Vector2f AT = ((distance / slideTime * playField.getOsuPx()) * dt);
            ...
            else if(sliderPointsCoords[0].y < initialPosition.y)
            {
                if( std::round(((this->hitCircle.getPosition() - playField.getPlayFieldStartPoint() )/playField.getOsuPx()).y) >= sliderPointsCoords[0].y && slides > 0 )
                {
                    sf::Vector2f AT = ((distance / slideTime * playField.getOsuPx()) * dt);
                    this->hitCircle.setPosition(this->hitCircle.getPosition() + AT);
